...
import ar.com.ada.api.cash.models.requests.LoanRequest;
import ar.com.ada.api.cash.models.responses.GenericResponse;
import ar.com.ada.api.cash.models.responses.InfoLoans;
import ar.com.ada.api.cash.models.responses.ItemLoan;
...
public class LoanController {
    ...
    public ResponseEntity<InfoLoans> getLoans(@RequestParam(value = "page", required = true) Integer page,
       ...
       /* esta es con la version MAP
        info.items = loans.stream().map(lo -> {
            ItemLoan iL = new ItemLoan();
            iL.id = lo.getId();
            iL.total = lo.getTotal();
            iL.userId = lo.getUser().getId();
            return iL;
